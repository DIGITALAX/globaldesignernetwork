...
const useMicrobrands = () => {
  ...
  const generateNonOverlappingPositions = (collections: Collection[]) => {
    ...
    // Detect screen size - use a reasonable assumption for SSR
    ...
    return collections.map((col, index) => {
      ...
      do {
        ...
        if (isMobile) {
          width = Math.random() * 60 + 120; // 120-180px instead of 180-280px
          height = Math.random() * 60 + 120;
          // Ensure left position keeps items within screen (with safety margin)
          x = Math.random() * Math.max(5, 85 - (width / 4)); // More conservative positioning
        } else {
      ...
      return {
        ...
        width,
        height,
        left: Math.max(0, Math.min(x, isMobile ? 85 : 95)), // Clamp to safe bounds
        top: Math.max(0, y),
      };
    });
  };
