...
const MarketEntry: FunctionComponent<{ dict: any }> = ({ dict }) => {
  ...
  return (
    <div className="relative flex flex-col">
      <Header dict={dict} />
      {drops?.[0] && (
        <div className="h-fit bg-gradient-to-br flex w-full from-[#f0f0f0] via-[#e8e8e8] to-[#f0f0f0] relative">
          <div className="flex flex-col mid:flex-row h-fit mid:h-[50rem] relative w-full">
            <div className="mid:border-r-4 border-black p-4 flex flex-col relative items-start justify-start h-fit mid:h-full w-full mid:w-1/3 bg-grey-200">
              <div
                className="relative w-full h-fit flex text-3xl mb-4 font-dos glitch-text"
              ...
              <div className="relative w-full h-fit flex flex-col gap-3">
                ...
                <div
                  ...
                  onClick={() =>
                    router.push(
                      `/designer/${featuredDrop?.designerProfile?.wallet}`
                    )
                  }
                >
                  {featuredDrop?.designerProfile?.metadata?.image && (
                    <div className="relative w-12 h-12 flex-shrink-0">
                      <Image
                        src={`${INFURA_GATEWAY}/ipfs/${featuredDrop.designerProfile.metadata.image?.split("ipfs://")?.[1]}`}
                        alt={featuredDrop.designerProfile.metadata.title || "Designer"}
                        layout="fill"
                        objectFit="cover"
                        draggable={false}
                  ...
                  <div className="relative flex flex-col">
                    ...
                    <div className="text-sm font-bold">
                      {featuredDrop?.designerProfile?.metadata?.title ||
                       `${featuredDrop?.designerProfile?.wallet?.slice(0, 6)}...${featuredDrop?.designerProfile?.wallet?.slice(-4)}`}
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="relative font-kare w-full h-fit flex flex-col bg-black gap-6 p-6">
          ...
          <div className="flex flex-wrap gap-3">
            <div ref={dropdownRef} className="relative w-full md:w-auto">
              <button
                ...
                className="relative w-full md:w-auto flex flex-row items-center justify-between gap-4 bg-white text-black px-4 py-2 border-2 border-black text-sm hover:bg-gray-100 transition-colors whitespace-nowrap break-all"
              >
              ...
              {dropdownOpen && (
                <div className="absolute top-full left-0 w-full md:w-64 mt-1 bg-white border-2 border-black shadow-lg z-50 max-h-60 overflow-y-auto">
                  <div
                    onClick={() => {
                      setSelectedDrop("all");
                      setDropdownOpen(false);
        ...
        <div className="relative w-full h-fit flex flex-col gap-2 px-6">
          ...
          <div className="relative w-full flex justify-center py-6">
            <button
              ...
              className="bg-black text-white px-6 py-2 border-2 border-black hover:bg-white hover:text-black transition-colors duration-300 disabled:opacity-50"
            >
